...
class PINN:
    ...
    def __init__(self,
                 Collo, P1, P2, P3, IC, LF, RT, UP, LW, DIST, 
        ...
        # Loading bound, where s12=0, s11=0
        self.x_RT = RT[:, 0:1]
        self.y_RT = RT[:, 1:2]
        self.t_RT = RT[:, 2:3]
        ...
        self.x_P1_tf = tf.placeholder(tf.float64, shape=[None, self.x_P1.shape[1]])
        self.y_P1_tf = tf.placeholder(tf.float64, shape=[None, self.y_P1.shape[1]])
        self.t_P1_tf = tf.placeholder(tf.float64, shape=[None, self.t_P1.shape[1]])
        self.v_P1_tf = tf.placeholder(tf.float64, shape=[None, self.v_P1.shape[1]])

        self.x_P2_tf = tf.placeholder(tf.float64, shape=[None, self.x_P2.shape[1]])
        self.y_P2_tf = tf.placeholder(tf.float64, shape=[None, self.y_P2.shape[1]])
        self.t_P2_tf = tf.placeholder(tf.float64, shape=[None, self.t_P2.shape[1]])
        self.v_P2_tf = tf.placeholder(tf.float64, shape=[None, self.v_P2.shape[1]])

        self.x_P3_tf = tf.placeholder(tf.float64, shape=[None, self.x_P3.shape[1]])
        self.y_P3_tf = tf.placeholder(tf.float64, shape=[None, self.y_P3.shape[1]])
        self.t_P3_tf = tf.placeholder(tf.float64, shape=[None, self.t_P3.shape[1]])
        ...
        self.P_u_IC_pred   , self.P_v_IC_pred   ,                     \
        self.P_s11_IC_pred , self.P_s22_IC_pred , self.P_s12_IC_pred ,\
        self.dt_P_u_IC_pred, self.dt_P_v_IC_pred                      = self.net_part(self.x_IC_tf, self.y_IC_tf, self.t_IC_tf)
        ##----------------------------------------------------------------
        self.P_u_LF_pred   ,                   _,                     \
        _                  ,                   _, self.P_s12_LF_pred ,\
        _                  ,                   _                      = self.net_part(self.x_LF_tf, self.y_LF_tf, self.t_LF_tf)
        ##----------------------------------------------------------------
        _                  ,                   _,                     \
        self.P_s11_RT_pred ,                   _, self.P_s12_RT_pred ,\
        _                  ,                   _                      = self.net_part(self.x_RT_tf, self.y_RT_tf, self.t_RT_tf)
        ##----------------------------------------------------------------
        _                  ,                   _,                     \
        _                  , self.P_s22_LW_pred , self.P_s12_LW_pred ,\
        _                  ,                   _                      = self.net_part(self.x_LW_tf, self.y_LW_tf, self.t_LW_tf)
        ##----------------------------------------------------------------
        _                  ,                   _,                      \
        _                  , self.P_s22_UP_pred , self.P_s12_UP_pred  ,\
        _                  ,                   _                      = self.net_part(self.x_UP_tf, self.y_UP_tf, self.t_UP_tf)
        ##----------------------------------------------------------------
        self.P_u_P1_pred   , self.P_v_P1_pred   ,                      \
        _                  , _                  ,                    _,\
        _                  ,                   _                      = self.net_part(self.x_P1_tf, self.y_P1_tf, self.t_P1_tf)
        ##----------------------------------------------------------------
        self.P_u_P2_pred   , self.P_v_P2_pred   ,                      \
        _                  , _                  ,                    _,\
        _                  ,                   _                      = self.net_part(self.x_P2_tf, self.y_P2_tf, self.t_P2_tf)
        ##----------------------------------------------------------------
        self.P_u_P3_pred   , self.P_v_P3_pred   ,                      \
        _                  , _                  ,                    _,\
        _                  ,                   _                      = self.net_part(self.x_P3_tf, self.y_P3_tf, self.t_P3_tf)
        ...
        self.loss_PART =   tf.reduce_mean(tf.square(self.P_u_IC_pred)) \
                         ...
                         + tf.reduce_mean(tf.square(self.P_s11_RT_pred)) \
                         + tf.reduce_mean(tf.square(self.P_s12_RT_pred)) \
                         + tf.reduce_mean(tf.square(self.P_s22_LW_pred)) \
                         ...
                         + tf.reduce_mean(tf.square(self.P_u_P1_pred  )) \
                         + tf.reduce_mean(tf.square(self.P_v_P1_pred - self.v_P1_tf)) \
                         + tf.reduce_mean(tf.square(self.P_u_P2_pred  )) \
                         + tf.reduce_mean(tf.square(self.P_v_P2_pred - self.v_P2_tf)) \
                         + tf.reduce_mean(tf.square(self.P_u_P3_pred  )) \
                         + tf.reduce_mean(tf.square(self.P_v_P3_pred  )) \

        self.loss = 10 * (self.loss_f_uv + self.loss_f_s)
    ...
    def net_f_sig(self, x, y, t):
        ...
        u_2   = tf.gradients(u  , y)[0]
        u_t   = tf.gradients(u  , t)[0]
        u_tt  = tf.gradients(u_t, t)[0]
        
        v_1   = tf.gradients(v  , x)[0]
        v_2   = tf.gradients(v  , y)[0]
        v_t   = tf.gradients(v  , t)[0]
        ...
        f_u = u_tt - ((self.S_star*self.T_star**2)/(self.rho*self.L_star*self.U_star))*s11_1 \
                   - ((self.S_star*self.T_star**2)/(self.rho*self.L_star*self.U_star))*s12_2
        #s11_1 + s12_2 - rho * u_tt
        f_v = v_tt - ((self.S_star*self.T_star**2)/(self.rho*self.L_star*self.U_star))*s22_2 \
                   - ((self.S_star*self.T_star**2)/(self.rho*self.L_star*self.U_star))*s12_1
        #s22_2 + s12_1 - rho * v_tt
        f_s11 = s11 - ((self.lambda0 + 2*self.mu0)*(self.U_star/(self.S_star*self.L_star)))*u_1 \
                    - ((self.lambda0             )*(self.U_star/(self.S_star*self.L_star)))*v_2
        
        f_s22 = s22 - ((self.lambda0 + 2*self.mu0)*(self.U_star/(self.S_star*self.L_star)))*v_2 \
                    - ((self.lambda0             )*(self.U_star/(self.S_star*self.L_star)))*u_1
        
        f_s12 = s12 -  self.mu0*(self.U_star/(self.S_star*self.L_star))*(u_2 + v_1)    
    ...
    def callback(self, loss, loss_f_uv, loss_f_s):
    ...
    def callback_part(self, loss_part, 
                      ICu_part, ICv_part, IC11_part, IC22_part, IC12_part, ICudt_part, ICvdt_part, 
                      LFu_part, LF12_part, RT11_part, RT12_part, LW22_part, LW12_part, UP22_part, UP12_part,
                      P1u_part, P1v_part, P2u_part, P2v_part, P3u_part, P3v_part):
        self.count = self.count + 1
        self.t1 = time.time()
        if self.count % 10 == 0:
    ...
    def train(self, iter, learning_rate):
        ...
        tf_dict = {self.x_c_tf: self.x_c, self.y_c_tf: self.y_c, self.t_c_tf: self.t_c,
                   ...
                   self.x_P1_tf: self.x_P1, self.y_P1_tf: self.y_P1, self.t_P1_tf: self.t_P1,
                   self.x_P2_tf: self.x_P2, self.y_P2_tf: self.y_P2, self.t_P2_tf: self.t_P2,
                   self.x_P3_tf: self.x_P3, self.y_P3_tf: self.y_P3, self.t_P3_tf: self.t_P3,
                   self.v_P1_tf: self.v_P1, self.v_P2_tf: self.v_P2,
                   ...
                   self.u_dist_tf: self.u_dist, self.v_dist_tf: self.v_dist, 
                   self.s11_dist_tf: self.s11_dist, self.s22_dist_tf: self.s22_dist, self.s12_dist_tf: self.s12_dist,
        ...
        for it in range(iter):
            ...
            self.it = self.it + 1
        return loss_f_uv, loss_f_s, loss#, iter + offset + 1
    ...
    def train_bfgs(self):
        ...
        self.optimizer.minimize(self.sess,
                                feed_dict=tf_dict,
                                fetches=[self.loss, self.loss_f_uv, self.loss_f_s],
